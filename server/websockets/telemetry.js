...
const cache = new NodeCache({ stdTTL: 60 }); // 60 секунд
...
async function handleTelemetry(data) {
  ...
  if (!inside) {
    console.warn(`⚠️ Drone ${droneId} exited the allowed zone`);
  }
}
...
function initWebSocketTelemetry(server) {
    ...
    wss.on('connection', (ws, req) => {
      ...
      ws.droneId = droneId; // сохранили в объекте соединения
      console.log(`🚁 WebSocket connected for drone ${droneId}`);
  
      ws.on('message', async (message) => {
        try {
          const data = JSON.parse(message);
          data.drone_id = droneId; // подставляем drone_id из URL
